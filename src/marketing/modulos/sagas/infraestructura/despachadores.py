...
from marketing.modulos.sagas.infraestructura.schema.v1.comandos.atribucion import (
    ComandoRevertirAtribucion,
    RevertirAtribucionPayload,
)
from marketing.modulos.sagas.infraestructura.schema.v1.comandos.interaccion import (
...
class DespachadorSagas:
    ...
    def publicar_comando_revertir_comision(self, comando):
        ...
        self._publicar_mensaje(
            comando_integracion, "revertir-comision-comando", ComandoRevertirComision
        )

    def publicar_comando_revertir_atribucion(self, comando):
        payload = RevertirAtribucionPayload(
            id_correlacion=comando.id_correlacion, journey_id=comando.journey_id
        )
        comando_integracion = ComandoRevertirAtribucion(data=payload)
        self._publicar_mensaje(
            comando_integracion, "revertir-atribucion", ComandoRevertirAtribucion
        )

    def publicar_comando_descartar_interacciones(self, comando):
        payload = DescartarInteraccionesPayload(
            id_correlacion=comando.id_correlacion, interacciones=comando.interacciones
        )
